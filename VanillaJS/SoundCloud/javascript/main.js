...
const SoundCloudAPI = {
  ...
  renderTracks: tracks => {
    ...
    tracks.forEach(track => {
      ...
      imageImg.src = track.artwork_url || "http://lorempixel/100/100/abstract";
      // Content
...
const UI = {
  ...
  setInputArea: () => {
    UI.inputArea.addEventListener("keyup", event => {
      if (event.which === 13) SoundCloudAPI.getTracks(UI.inputArea.value);
    });
  },
  setSearchButton: () => {
...
UI.setPlayList();
UI.setResetButton();
// SoundCloudAPI.getTracks("maroon5");
